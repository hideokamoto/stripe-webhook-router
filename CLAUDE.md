# CLAUDE.md - Tayori Development Guide for AI Assistants
...
- First-class Stripe support with 253+ event types
- Schema validation support via Zod integration
- Monorepo structure using pnpm workspaces
- Node.js >= 18 required

**Repository**: `hideokamoto/stripe-webhook-router`
**Default Branch**: `main`
...
### 1. @tayori/core (`packages/core/src/index.ts` - 290 lines)
...
**Tests**: `packages/core/test/webhook-router.test.ts` (comprehensive unit tests)

### 2. @tayori/stripe (`packages/stripe/src/index.ts` - 438 lines)
...
- `StripeEventMap` - Type map of all 253+ Stripe event types
...
### 4. create-tayori (`packages/create-tayori/src/` - 73 lines main)
...
```
src/
├── index.ts (73 lines)           # Main orchestration
...
### 5. @tayori/hono (`packages/hono/src/index.ts` - 90 lines)
...
### 6. @tayori/express (`packages/express/src/index.ts` - 106 lines)
...
### 7. @tayori/lambda (`packages/lambda/src/index.ts` - 111 lines)
...
### 8. @tayori/eventbridge (`packages/eventbridge/src/index.ts` - 48 lines)
...
```bash
# Clone repository
git clone https://github.com/hideokamoto/stripe-webhook-router.git
...
- **Required**: pnpm 9.15.0 (enforced by `packageManager` field)
- Workspace dependencies use `workspace:*` protocol
- Lock file: `pnpm-lock.yaml` (must be kept in sync)
...
- **Coverage**: Use `pnpm test:coverage` for coverage reports
...
- Jobs:
  ...
  2. **Type Check**: Runs TypeScript type checking (builds core first)
  3. **Test**: Runs tests on Node 22 and 24
  4. **Build**: Builds all packages (runs after all checks pass)

**Test Matrix**:
- Node versions: 22, 24
- OS: ubuntu-latest

**Important**: Core package must be built before type checking and tests due to workspace dependencies.

### Pre-commit Hooks

**Husky** (`.husky/pre-commit`):
- Runs `pnpm exec lint-staged`
- Configured in `package.json` `lint-staged` field
- Runs ESLint on modified TypeScript files in `packages/*/src/**/*.{ts,tsx}`
...
- **Feature Branches**: Use descriptive names
  - For Claude Code: `claude/feature-description-SESSION_ID`
...
- `core`, `stripe`, `zod`, `hono`, `express`, `lambda`, `eventbridge`, `create-tayori`
- Omit scope for root-level changes or use `repo`

**Examples**:
```
feat(stripe): add support for new webhook events
fix(hono): handle missing headers gracefully
feat(zod): add schema validation middleware
...
- Key Rules:
  ...
  - No duplicate class members (TypeScript handles overloads)
...
2. **Initialize package.json**:
   - Use `@tayori/core` as `workspace:*` dependency
   - Add framework as peer dependency
   - Include standard scripts: `build`, `test`, `test:watch`, `typecheck`
...
7. **Add to CONTRIBUTING.md** package list
...
3. **Create Generator** (`packages/create-tayori/src/generators/framework-name.ts`):
   - Import utilities from `../utils/files`
   - Implement template copying and variable replacement
...
6. **Update Documentation**: Update event count in CLAUDE.md and README.md
...
// Define event schemas
const paymentSucceeded = defineEvent('payment_intent.succeeded', z.object({
  id: z.string(),
...
- `packages/*/src/index.ts` - Adapter implementations

### Test Files

- `packages/core/test/webhook-router.test.ts` - Core tests
- `packages/*/test/*.test.ts` - Package-specific tests
...
- `packages/stripe/MAINTAINING_STRIPE_EVENTMAP.md` - Event map maintenance
...
**Solution**: Ensure schemas are registered in SchemaRegistry before use

**Issue**: CI failing on type check
**Solution**: Core package must be built first (see CI workflow)
...
10. **Backward Compatibility**: How does this affect existing users?
...
- `packages/stripe/src/index.ts` - Stripe event map
- `packages/zod/src/index.ts` - Zod validation helpers
